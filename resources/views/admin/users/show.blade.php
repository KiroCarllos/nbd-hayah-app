            ...
            <div class="card mb-4">
                <div class="card-body text-center">
                    @if ($user->profile_image)
                        <img src="{{ asset('storage/' . $user->profile_image) }}" alt="Profile" class="rounded-circle mb-3"
                            width="120" height="120" style="object-fit: cover;">
                    @else
                        <div class="bg-primary d-flex align-items-center justify-content-center rounded-circle mx-auto mb-3"
                ...
                <div class="card-body">
                    <div class="row text-center">
                        <div class="col-6">
                            ...
                            <small class="text-muted">ج.م رصيد المحفظة</small>
                        </div>
                        <div class="col-6">
                    ...
                    <div class="row text-center">
                        <div class="col-6">
                            ...
                            <small class="text-muted">ج.م إجمالي التبرعات</small>
                        </div>
                        <div class="col-6">
                                                ...
                                                <div class="d-flex align-items-center">
                                                    @if ($donation->campaign->images && count($donation->campaign->images) > 0)
                                                        <img src="{{ asset('storage/' . $donation->campaign->images[0]) }}"
                                                            alt="Campaign" class="rounded me-3" width="40"
                                                    ...
                                                    <div>
                                                        ...
                                                        <br>
                                                        <small
                                                            class="text-muted">{{ $donation->campaign->creator->name }}</small>
                                                    </div>
                                                </div>
                                            </td>
                                            <td>
                                    ...
                                    @foreach ($user->walletTransactions->take(10) as $transaction)
                                        <tr>
                                            ...
                                            <td>
                                                @if ($transaction->type === 'charge')
                                                    <span
                                                        class="text-success fw-bold">+{{ number_format($transaction->amount, 2) }}
                                                        ج.م</span>
                                                @else
                                                    <span
                                                        class="text-danger fw-bold">-{{ number_format($transaction->amount, 2) }}
                                                        ج.م</span>
                                                @endif
                                            </td>
                                            <td>{{ number_format($transaction->previous_balance, 2) }} ج.م</td>
                                            <td>{{ number_format($transaction->new_balance, 2) }} ج.م</td>
                                            <td>
                                        ...
                                        <div class="row g-0">
                                            <div class="col-4">
                                                @if ($favorite->campaign->images && count($favorite->campaign->images) > 0)
                                                    <img src="{{ asset('storage/' . $favorite->campaign->images[0]) }}"
                                                        class="img-fluid rounded-start h-100"
                                                        alt="{{ $favorite->campaign->title }}"
                                                        style="object-fit: cover;">
                                                @else
                                                    <div
                                                        class="bg-primary d-flex align-items-center justify-content-center rounded-start h-100">
                                                        <i class="bi bi-heart-fill text-white"></i>
                                                    </div>
                                                @endif
                                            </div>
                                            <div class="col-8">
                                                <div class="card-body p-2">
                                                    <h6 class="card-title">
                                                        {{ \Illuminate\Support\Str::limit($favorite->campaign->title, 30) }}
                                                    </h6>
                                                    <small
                                                        class="text-muted">{{ number_format($favorite->campaign->progress_percentage, 1) }}%
                                                        مكتمل</small>
                                                    <div class="progress" style="height: 4px;">
                                                        <div class="progress-bar bg-success"
                                                            style="width: {{ $favorite->campaign->progress_percentage }}%">
                                                        </div>
                                                    </div>
                                                </div>
                                            </div>

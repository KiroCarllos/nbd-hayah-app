    ...
    <div class="col-md-3">
        <div class="card stats-card bg-info text-white">
            <div class="card-body">
                <div class="d-flex justify-content-between">
                    <div>
                        <h4>{{ number_format($campaigns->sum('current_amount'), 0) }} ر.س</h4>
            ...
            <table class="table table-hover">
                ...
                <tbody>
                    ...
                    <tr>
                        <td>
                            <div class="d-flex align-items-center">
                                @if($campaign->images && count($campaign->images) > 0)
                                    <img src="{{ asset('storage/' . $campaign->images[0]) }}" alt="Campaign" class="rounded me-3" width="50" height="50" style="object-fit: cover;">
                                @else
                                    <div class="bg-primary d-flex align-items-center justify-content-center rounded me-3" style="width: 50px; height: 50px;">
                        ...
                        <td>
                            <div class="d-flex align-items-center">
                                @if($campaign->creator->profile_image)
                                    <img src="{{ asset('storage/' . $campaign->creator->profile_image) }}" alt="Creator" class="rounded-circle me-2" width="30" height="30">
                                @else
                        ...
                        <td>{{ number_format($campaign->target_amount, 2) }} ر.س</td>
                        <td>{{ number_format($campaign->current_amount, 2) }} ر.س</td>
                        <td>
                        ...
                        <td>
                            <div class="btn-group" role="group">
                                <a href="{{ route('admin.campaigns.show', $campaign) }}" class="btn btn-sm btn-outline-info">
                                    <i class="bi bi-eye"></i>
                                </a>
                                <a href="{{ route('admin.campaigns.edit', $campaign) }}" class="btn btn-sm btn-outline-primary">
                                    <i class="bi bi-pencil"></i>
                                </a>
                                <form action="{{ route('admin.campaigns.destroy', $campaign) }}" method="POST" class="d-inline" onsubmit="return confirm('هل أنت متأكد من حذف هذه الحملة؟')">
                                    @csrf
                                    @method('DELETE')

 ...
 * Handles wallet password verification and setting
 */
...
class WalletPasswordManager {
    ...
    /**
     * Check if user has wallet password and show appropriate modal
     * @param {Function} callback - Function to call after successful verification
     * @param {string} action - Action description for user
     */
     ...
     */
    showPasswordModal() {
        const modal = new bootstrap.Modal(document.getElementById('walletPasswordModal'));
        modal.show();
    }
    ...
    showSetPasswordModal() {
        const modal = new bootstrap.Modal(document.getElementById('setWalletPasswordModal'));
        modal.show();
    }

    /**
     * Handle wallet password form submission
    ...
    async handleWalletPasswordSubmit(form) {
        ...
        try {
            ...
            if (data.success) {
                ...
                bootstrap.Modal.getInstance(document.getElementById('walletPasswordModal')).hide();
                ...
                errorDiv.textContent = data.message || 'كلمة مرور المحفظة غير صحيحة';
            }
        } catch (error) {
    ...
    async handleSetWalletPasswordSubmit(form) {
        ...
        try {
            ...
            if (data.success) {
                // Hide modal and execute callback
                bootstrap.Modal.getInstance(document.getElementById('setWalletPasswordModal')).hide();
                this.showSuccess('تم تعيين كلمة مرور المحفظة بنجاح');
                if (this.currentCallback) {
                    this.currentCallback();
                }
            } else {
                this.showError(data.message || 'حدث خطأ في تعيين كلمة مرور المحفظة');
            }
        } catch (error) {
